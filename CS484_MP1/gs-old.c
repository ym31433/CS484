  ...
  for(it = 0; it != MAXIT; ++it) {
    ...
    if(threadX-1 >= 0) {
      while(barrier[up] != it*2) {
        #pragma omp flush (barrier[up])
      }
      for(f_it = idy_start; f_it != idy_end; ++f_it) {
        #pragma omp flush (A[IND(idx_start-1, f_it, p_asize)])
      }
    }
    //down
    if(threadX+1 < num_tile_x) {
      while(barrier[down] != it*2) {
        #pragma omp flush (barrier[down])
      }
      for(f_it = idy_start; f_it != idy_end; ++f_it) {
        #pragma omp flush (A[IND(idx_end+1, f_it, p_asize)])
      }
    }
    //left
    if(threadY-1 >= 0) {
      while(barrier[left] != it*2) {
        #pragma omp flush (barrier[left])
      }
      for(f_it = idx_start; f_it != idx_end; ++f_it) {
        #pragma omp flush (A[IND(f_it, idy_start-1, p_asize)])
      }
    }
    //right
    if(threadY+1 < num_tile_y) {
      while(barrier[right] != it*2) {
        #pragma omp flush (barrier[right])
      }
      for(f_it = idx_start; f_it != idx_end; ++f_it) {
        #pragma omp flush (A[IND(f+it, idy_end+1, p_asize)])
      }
    }
    ...
    for(i = idx_start; i != idx_end; ++i) {
      for(j = !alt ? idy_start: idy_start+1; j < idy_end; j+=2) {
        ...
        A[IND(i,j-1,p_asize)] +
        A[IND(i,j+1,p_asize)]);
	#pragma omp flush (A[IND(i, j, p_asize)])
      }
      alt = !alt;
    }
    ...
    //update barrier
    barrier[threadID]++;
    #pragma omp flush (barrier[threadID])
